...
- dir: Path to the PDF file to send
...
def main():
    if len(sys.argv) < 2:
        print("Usage: python send_whatsapp_files.py <path_to_csv_or_xlsx>")
        print("Example: python send_whatsapp_files.py contacts.xlsx")
        sys.exit(1)
    ...
    # Filter out empty rows (where celular or dir is empty/NaN)
    df = df.dropna(subset=['celular', 'dir'])
    ...
    print("   - Open the chat with a pre-filled intro message")
    print("   - Copy the PDF file to your clipboard")
    print("3. You need to:")
    print("   - Press Enter or Send (to send the intro message)")
